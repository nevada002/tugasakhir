...
<body>
    ...
    <ol style="padding-left: 17;">
        <li>Menunjuk Nota Penjualan Kepelabuhan Pelayanan Sampah Kapal yang terbit tanggal
            {{ $hasil->berita_acara->tanggalnotasampahkapal->locale('id')->isoFormat('DD MMMM YYYY') }} sebagaimana
            Daftar terlampir.</li>
        <li>Setelah diadakan penelitian dapat dijelaskan bahwa terdapat kesalahan sistem yang membuat Penerimaan Pajak
            Negara (PPN) tidak tertagih pada Nota Penjualan Jasa Kepelabuhan Pelayanan Sampah Kapal yang terbit tanggal
            {{ $hasil->berita_acara->tanggalnotasampahkapal->locale('id')->isoFormat('DD MMMM YYYY') }}</li>
        <li>Sehubungan butir 2 (dua) tersebut di atas, dimohon agar dapat dilakukan koreksi pada sistem Nota Penjualan
            Jasa Kepelabuhan Pelayanan Sampah Kapal yang terbit tangagal
            {{ $hasil->berita_acara->tanggalnotasampahkapal->locale('id')->isoFormat('DD MMMM YYYY') }} dan agar
            dimunculkan Pajak Penerimaan Negara (PPN) pada Nota-nota sampah kapal tersebut.</li>
    </ol>
