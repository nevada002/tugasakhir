...
<body>
    ...
    <ol style="padding-left: 17;">
        <li>Berdasarkan surat {{ $hasil->berita_acara->nama_perusahaan }}, No:
            {{ $hasil->berita_acara->no_surat_perusahaan }}, tanggal
            {{ $hasil->berita_acara->tanggal_surat->locale('id')->isoFormat('dddd MMMM YYYY') }}, perihal:
            {{ $hasil->berita_acara->perihal }}</li>
        <li>
            Setelah diadakan penelitian dan evaluasi atas permohonan koreksi tersebut dapat dijelaskan sebagai berikut:
            {{ $hasil->berita_acara->nota->deskripsi }}
        </li>
        <li>Sehubungan butir 2 (dua) tersebut di atas, maka permohonan koreksi atas Nota Kapal Nomor:
            {{ '012038038' }}, untuk koreksi pasal 5.2b SK. Dir dapat disetujui.</li>
    </ol>
