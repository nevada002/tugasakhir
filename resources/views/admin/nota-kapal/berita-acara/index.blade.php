        ...
        <tbody>
            @foreach ($beritaAcaras as $data)
                <tr>
                    ...
                    <td>{{ $data->tanggal_surat->locale('id')->isoFormat('DD MMMM YYYY') }}</td>
                    <td>{{ $data->perihal }}</td>
                    ...
                    <td class="justify-content-space-between w-100">
                        <a href="{{ route('admin.nota-kapal.berita-acara.show', ['id' => $data->id]) }}"
                            class="btn btn-primary">
                            <i class="bi bi-eye"></i>
                        </a>

                        @if (auth()->user()->isCustomerService())
                            <a href="{{ route('admin.nota-kapal.berita-acara.edit', ['id' => $data->id]) }}"
                        ...
                        @if ((auth()->user()->isSigner() &&
                            ...
                            <button onclick="approval(this)" class="btn btn-secondary ms-1"
                                data-url="{{ route('admin.nota-kapal.berita-acara.approval', $data->id) }}"
